...
function get_accueil( $data ) {
	$page = get_page_by_title('Bienvenue', OBJECT, 'page');
...
add_action( 'rest_api_init', function () {
	register_rest_route( 'wp/v2', '/accueil', array(
	  'methods' => 'GET',
	  'callback' => 'get_accueil',
	) );
  } );

function get_catalog( $data ) {
	$parent_page_id = get_page_by_title('Catalogue', OBJECT, 'page')->ID;
...
function get_product( $data ) {
	$product = get_page_by_path('catalogue/'.$data['category'].'/'.$data['product'], OBJECT, 'page');
...
function get_photo( $data ) {
	$page =  get_page_by_title('Catalogue', OBJECT, 'page');
...
function get_galerie() {
	$page =  get_page_by_title('Galerie', OBJECT, 'page');
...
add_action( 'rest_api_init', function () {
 register_rest_route( 'wp/v2', '/galerie', array(
   'methods' => 'GET',
   'callback' => 'get_galerie',
 ) );
} );

function get_order() {
	$page =  get_page_by_title('Commander', OBJECT, 'page');
