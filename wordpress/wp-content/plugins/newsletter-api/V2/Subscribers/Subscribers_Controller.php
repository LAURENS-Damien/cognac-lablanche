...
class Subscribers_Controller extends REST_Controller {
	...
	public function create_item( $request ) {
		...
		$subscription = $this->create_TNP_subscription_object( $request );
		$subscriber   = NewsletterSubscription::instance()->subscribe2( $subscription );
	 ...
	 */
	private function create_TNP_subscription_object( $request ) {
	 ...
	 * @param WP_REST_Request $request
	 *
	 * @return TNP_Subscription_Data
	 */
	protected function create_subscription_data_from_request( $request ) {
		$subscriber = new TNP_Subscription_Data();

		$subscriber->email    = NewsletterModule::normalize_email( $request->get_param( 'email' ) );
		$subscriber->name     = NewsletterModule::normalize_name( $request->get_param( 'first_name' ) );
		$subscriber->surname  = $request->get_param( 'last_name' );
		$subscriber->sex      = NewsletterModule::normalize_sex( array_search( $request->get_param( 'gender' ), self::SUBSCRIBER_SEX_ENUM ) );
		...
		//$subscriber->status   = $request->get_param( 'status' );

		$lists = $request->get_param( 'lists' );
		if ( $lists ) {
			foreach ( $lists as $list ) {
				$subscriber->lists[ (int) $list['id'] ] = (int) $list['value'];
			}
		}
		...
		if ( $extra_fields ) {
			foreach ( $extra_fields as $field ) {
				$subscriber->profiles[ (int) $field['id'] ] = sanitize_text_field( $field['value'] );
			}
		}
